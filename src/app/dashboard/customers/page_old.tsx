...
export default function Page_old(): React.JSX.Element {
...
async function fetchCustomers(): Promise<Customer[]> {
  ...
  const responseCustomers = await customersClient.getCustomers();
  console.log('responseCustomers - ', responseCustomers?.data)
  // Например, использование fetch или axios
  // Возвращаемые данные должны соответствовать типу Customer[]
  // Временно заглушаем данные для демонстрации
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve([...customers]); // Заглушка, замените эту строку на реальный код получения данных с сервера
    }, 1000); // Имитируем задержку сети для наглядности
  });
}
